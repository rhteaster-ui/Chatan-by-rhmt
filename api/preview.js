import { sendJson } from './_telegram.js';
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const target = new URL(Array.isArray(rawUrl) ? rawUrl[0] : rawUrl);
    if (!['http:', 'https:'].includes(target.protocol)) return sendJson(response, { error: 'URL tidak valid.' }, 400);

    const previewResponse = await fetch(target.toString(), {
      headers: { 'user-agent': 'R-HMT-OFC-LinkPreview/1.0' },
      signal: AbortSignal.timeout(6000),
    });
    const html = (await previewResponse.text()).slice(0, 300000);
    ...
    return sendJson(response, { title, description, image, siteName });
  } catch {
    return sendJson(response, { error: 'Preview tidak tersedia.' }, 400);
  }
}
