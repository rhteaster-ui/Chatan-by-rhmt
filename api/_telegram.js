...
const statusPattern = /^(?:\/status(?:@\w+)?|#STATUS)\s+(OPEN|CLOSED)\s*$/i;
...
export function chatMatchesConfiguredGroup(update, chatId) {
  const message = update.message || update.channel_post || update.edited_message;
  return String(message?.chat?.id || '') === String(chatId);
}

export function parseTelegramStatus(update) {
  const message = update.message || update.channel_post || update.edited_message;
  const text = message?.text || '';
...
export async function getLatestSessionStatus() {
  ...
  const latestStatus = updates
    .filter((update) => chatMatchesConfiguredGroup(update, chatId))
    .map(parseTelegramStatus)
    .filter(Boolean)
    .sort((left, right) => new Date(right.createdAt || 0) - new Date(left.createdAt || 0))[0];

  return {
    status: latestStatus?.status || 'OPEN',
    source: latestStatus ? 'telegram-command' : 'default',
    updatedAt: latestStatus?.createdAt || null,
  };
}
