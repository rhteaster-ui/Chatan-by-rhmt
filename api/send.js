import { ensureTelegramConfig, getLatestSessionStatus, readJson, sendJson, telegram, validateMessage } from './_telegram.js';
...
export default async function handler(request, response) {
  ...
  try {
    ...
    const currentStatus = await getLatestSessionStatus();
    if (currentStatus.status === 'CLOSED') {
      return sendJson(response, { error: 'Sesi chat sedang ditutup dari command Telegram.' }, 403);
    }
