...
const SESSION_STATUS = import.meta.env.VITE_SESSION_STATUS || 'OPEN';
...
function ChatApp() {
  ...
  const isClosed = SESSION_STATUS.toUpperCase() === 'CLOSED';
  ...
  useEffect(() => {
    loadMessages();
    const timer = setInterval(loadMessages, 20000);
    ...
    <main className="mx-auto flex min-h-screen max-w-6xl flex-col px-5 py-8">
      <header className="flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-glow backdrop-blur-lg md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="mt-2 text-zinc-300">Halo, <span className="font-bold text-neon">{session.username}</span>. Public feed ditarik dari Telegram.</p>
        </div>
